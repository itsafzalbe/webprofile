...
// Use the same base as the API client — works in both dev and prod
const API_BASE = (import.meta.env.VITE_API_URL || "/api/v1")
  .replace("/api/v1", "")  // strip the path, keep the origin

function mediaUrl(path) {
  if (!path) return null
  if (path.startsWith("http")) return path   // already absolute
  return `${API_BASE}${path}`                // e.g. /static/avatar.jpg → https://afzalbe.uz/static/avatar.jpg
}
...
export default function About() {
  ...
  // Social links — handle both array [{platform, url}] and object {github: "..."}
  const socialEntries = (() => {
    const links = profile?.social_links
    if (!links) return []
    if (Array.isArray(links)) return links.map(l => [l.platform, l.url])
    ...
    <div style={s.page}>
      ...
      {profile?.bio && (
        ...
        >
          ...
          <p style={{ fontSize:"15px", lineHeight:1.8, color:t.textMuted, maxWidth:"640px", margin:0 }}>
            {profile.bio}
          </p>
        </motion.section>
      )}
          ...
          <div style={{ display:"flex", flexWrap:"wrap", gap:"8px" }}>
            {profile.skills_summary.map((skill, i) => (
              <span key={i} style={{
                ...
                color:t.text, fontWeight:500, letterSpacing:"0.01em",
              }}>
                {skill}
              </span>
            ))}
      ...
      >
        ...
        {achLoading ? (
          ...
          <div style={s.cardGrid}>
            {sortedAchs.map((ach, i) => (
              <AchCard key={ach.id || i} ach={ach} t={t} s={s} />
            ))}
          </div>
        )}
      </motion.section>
    ...
    >
      ...
      <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:"10px" }}>
        <span style={{
          ...
          background:cat.bg, color:cat.text, letterSpacing:"0.04em", textTransform:"uppercase",
        }}>
          {cat.label}
        </span>
      ...
      {/* Achievement image — same mediaUrl fix */}
      {mediaUrl(ach.image_url) && (
        <img
          src={mediaUrl(ach.image_url)}
          alt={ach.issuer}
          style={{ width:"32px", height:"32px", objectFit:"contain", marginBottom:"10px", borderRadius:"6px" }}
        />
      )}

      <div style={{ fontSize:"14px", fontWeight:600, color:t.text, marginBottom:"4px", lineHeight:1.3 }}>
        {ach.title}
      </div>
      <div style={{ fontSize:"12px", color:t.accent, marginBottom:"6px" }}>{ach.issuer}</div>

      {ach.description && (
        <div style={{ fontSize:"12px", color:t.textMuted, lineHeight:1.6, marginBottom:"10px" }}>
          {ach.description.slice(0,100)}{ach.description.length > 100 ? "…" : ""}
        </div>
      )}

      {ach.skills?.length > 0 && (
        <div style={{ display:"flex", flexWrap:"wrap", gap:"4px", marginBottom:"10px" }}>
          {ach.skills.slice(0,4).map(skill => (
